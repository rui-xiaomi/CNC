...
7. **【2026-08 收紧】预记/落账统一**。料架路径 Redo / Redispatch / AutoRedo 发 RCS 前：已有预记跳过，已回滚须再预记，失败拒发（ADR-0002）。COMPLETED 落账与对账①b 走同一决策：HasMat 未知不 Confirm 也不 Rollback、不放工位；明确不符才回滚+告警；未绑加工位 COMPLETED 按方向直接落账。见 §6.1 / §6.3 / §6.5。
8. **【2026-09 现场】出 OK/NG 先写 `POS_TEST_START=2` 关启动，再入下料队。** 启动是电平，1 保持则机台反复测；下料失败进 Alarm 时也不得继续测。UNLOADED 再写一次 2 作幂等收口。见 §6.1 / §7。
9. **【2026-09 现场】RCS 取消后在监控看板收口。** 告警工位点「恢复」时一并记 `CANCEL_MANUAL_FLAG=1`；仅占用锁、未告警时点「确认取消」。不必再到 RCS 任务页点「取消任务 / 确认取消已处理」。见 §5.5 / §8。
...
| 与机台 | **不直连**，一切经 PLC |
| 与 RCS | **【v2 变更】HTTP/JSON 正式对接**：出站 4 接口（transitTask / excuteTask / cancelTask / queryTask），入站 3 回调（pushTaskStatus / scanTaskStatus / warnCallback)。**【v2.1 已确认】搬运任务完成也经 pushTaskStatus（3.6）推送**——任务跟踪以**回调为主通道，queryTask 轮询兜底**；transitTask 的 taskType 按契约填 `out`（出库/上料）、`in`（入库/下料）、`move`（换架/空托盘/转序）。工位上下料默认走 `excuteTask`/`grabTask`（`App:Rcs:LoadUnloadVerb`，见 ADR-0004） |
...
实现"物料A ↔ 中转架 ↔ 1层2位"的绑定与随时反查。槽位带**层 + 层内位**两级物理坐标（如上料架 10 层 × 左右 2 位 = 20 槽）。
...
| 表 | 用途 |
| --- | --- |
| `MAS_AUTO_AGV_TASK`（扩展） | 增加 `RCS_TASK_ID`（本地号）、`RCS_REMOTE_ID`（回包号）、`RCS_STATUS`（RCS 原始 11 态）、`DISPATCH_TIME`、`FINISH_TIME`、`REDO_COUNT`、`CANCEL_MANUAL_FLAG`（取消后人工处理确认标记） |
| `MAS_AUTO_LOCATION_MAP`（新增） | 逻辑位置（机台/加工位/料架位）↔ RCS 点位编码（`RCS_CODE` + `RCS_TYPE`：shelf/cell/station）。**【现场搬运】料架 cell = 货架码+层编码（第1层=10、第2层=11…）+层内位1位（10层×左右2：上料 101101/101102、101111/101112…101191/101192；中转 301/302、下料 303、NG 401 同规则）；加工位按录入（内长宽 201101/201102）；抓取仍用 station/shelf（101/201）。禁止 `LOAD_AREA=101` 与上料架 shelf 同码** |
...
2. **读操作界面**：先选 PLC（或选机台自动定位其 PLC）→ 读取该 PLC 全部映射点位（含写信号）→ 显示原始值与按 ON/OFF 翻译后的语义值。支持单次读与周期轮询。点位映射「写」勾选不从读面板移除该行。
3. **写操作界面**：选 PLC/机台 → 选映射表中任一信号（不按点位「写」勾选互斥）→ 选写入值（启动=1/关闭=2）→ 下发 → 回读校验。**写操作二次确认 + 落操作流水**（影响现场设备）。读面板同步展示该 PLC 全部映射点位（含写信号）。
...
1. **连接配置与健康状态**：RCS 地址/端口/clientCode、本机回调监听端口；健康检查 = 定时空 queryTask，状态灯展示。配置存 `MAS_AUTO_WORKLINE_AGV`，口令加密。`Rcs.UseSimulator=true` 时出站 BaseUrl 强制改写本机环回，不吃库内现场地址。
2. **任务列表**：本地任务 ↔ `RCS_TASK_ID`、RCS 原始 11 态、下发/完成时间、耗时；操作按钮：手动下发（测试用）/ **redo**（同 taskId 重发，`REDO_COUNT`+1）/ **cancel**。
3. **取消工单**：cancelTask / RCS 侧取消后生成人工工单（接口文档明确：取消后小车与容器需人工处理）；`CANCEL_MANUAL_FLAG` 未确认前，**锁定相关点位禁止重新派工**。操作员在监控看板确认：告警工位点「恢复」（恢复成功时一并记确认）；仅占用锁、未告警时点「确认取消」。RCS 页「确认取消已处理」仍可用，不是必经步骤。
...
**(b) 物料位置追踪（分层）**
- 料架建档：填层数与每层数（如 10 层 × 2），系统按 `层×位` 预建全部空槽。
...
flowchart TD
    ...
    R -->|OK| RecOK["记录结果=OK"]
    R -->|NG| RecNG["记录结果=NG"]
    RecOK --> Stop["写 POS_TEST_START=2 关闭启动"]
    RecNG --> Stop
    Stop --> UnloadD["下发 RCS 下料任务<br/>(加工位→下料架)"]
    UnloadD --> WaitRcs2["等待 RCS 完成"]
    WaitRcs2 --> Verify2{"PLC 复核<br/>有料=OFF"}
    Verify2 -->|通过| Ready["加工位回到等待上料<br/>槽位账按 §4.3 机制更新<br/>再写 POS_TEST_START=2（幂等）"]
    Verify2 -->|不通过| Hold
    Ready --> Poll
```
...
| 3 | 排队+下发 | 队列按 priority 出队（**紧急下料 > 常规上料**，RCS 单任务串行）；LOCATION_MAP 翻译点位（抓取 station / 搬运 cell）；按 `LoadUnloadVerb` 调 excuteTask/grabTask 或 transitTask（taskType=`out`/`in`/`move`）；换架/空托盘固定 transitTask + `move`。Success → DISPATCHED，状态机进 DISPATCHING；网络失败重试 ≤3，仍失败告警暂停派工 | RcsClient / LOCATION_MAP, RCS_MSG_LOG |
...
| 7 | 加工监视 | 轮询 OK/NG，出结果写加工记录，**立刻写 POS_TEST_START=2 关启动**（电平保持 1 则机台反复测） | WORK_RECORD / PLC |
| 8 | 下料闭环 | 关启动后再下发下料（加工位→下料架/NG 架），重走 2~5 步；完成 + PLC 复核有料=OFF → 再写 POS_TEST_START=2（幂等）→ WAIT_LOAD | 同上 |

补充规则：
- **槽位账时序**：第 2 步选定孔位时"预记"（占用意向，须先于 RCS，见 ADR-0002），第 6 步回调确认且 PLC 符合阶段预期后"落账"。取消/失败回滚预记。Redo / Redispatch / AutoRedo：已有预记幂等跳过，已回滚须先再预记再下发；预记失败拒发；本轮新锁且下发失败才回滚。上料有物料码只锁该件，找不到不得改抢其它槽。命名区 / 换架 / 手动无槽抓取 / identify 无槽位账则跳过；自动 grab 上下料有槽账，Redo 按 Kind=grab 补预记。手工无 taskId 首次下发不在此列。
...
**工序路由链**：上料区 → 内长宽 → A基准 → 平面度 → 下料区；任意工序 NG → NG 架（不向后流转）。

**下料目标动态决策**（§6.1 第 8 步细化）：
- **【v2.8 / 2026-09 现场】无工位直送 + 一架两用**。工序 n 完成且结果 OK、且有活动下一工序 → **送本机下料架（role1）**，该架同时是下一工序上料架（role0）。本机未绑下料架时才看下游上料架 / 旧 role2。下一工序已配置但全部不可用 → 拒发，禁止命名区伪造成功。
- 结果 NG → 送 NG 架，工件记录标记 NG、生成待处理项。
- **【v2.4 实现】终点决策与预记同在单一调度消费者内串行完成**，避免上游多件同时 OK 时抢占同一共享架槽。

**上料来源决策**（§6.1 第 2 步细化）：
- 工序 n 请求上料时取**本机上料架（role0）**；无上料绑定才回退旧 role2。架空则等待上游入架（不从命名区自取原料）。
- **【v2.4 实现】工位不自查料，仅投递"上料请求"；由单一调度消费者串行查料源→原子预占→下发；料源无料则保持 WAIT_LOAD 等待（不告警），详见 §6.4。**
- **【v2.5】在途直送与中转自取互斥**：目标工位已有 `_expectedInbound`（上游直送在途）时，禁止该位从中转架/上料架自取；选下游空位时排除已 `UploadRequested` 的工位。废弃交接登记时禁止静默丢件。~~物料退回目标机中转架~~ **【2026-09 修订】废弃交接只落严重告警（物料码、源任务、目标工位、原因），不在任何料架记账**：实物可能在车上、源工位或目标工位，直接记中转架占用会凭空多一件；由人工确认实物后在料架页校正。
- **【v2.6】陈旧交接与预记失败收口**：清除在途直送登记仅当源任务为 CANCELED/缺失或超过 TTL——**不含 COMPLETED**（避免 RCS 完成与 PLC 见料窗口内误退回中转）。**【2026-09 修订】不含 FAILED**：FAILED 会触发自动 redo，车仍可能按原任务送达；FAILED 源任务的登记走 TTL，或在任务被放弃（redo 达上限 / 查无）时清除。RCS 已下发但入架/取料预记失败，或上料下发窗口被直送抢占时，须 cancel 并源工位 ALARM，禁止 orphan 任务挂起；**【2026-09 修订】cancel 失败不落库 FAILED、不回滚预记**（车可能仍在执行），落告警并交跟踪器按 queryTask 收敛。
- **【2026-09 修订 · P1-6】交接登记清理进工位闸**：按源任务清除直送登记（CANCELED 回调、任务放弃、源工位告警/人工恢复）不在调用处直接删，只登记清理请求，由**目标工位**下一轮驱动在其工位闸内执行，且仅当该位登记的源任务仍是请求中的任务才清（同位新登记不被误删）。与目标工位见料消费、自取判定串行；最多延迟一个调度周期。
- **【v2.7 / 2026-08】COMPLETED 预记 PLC 门 + 交接宽限**：运行期/对账对「任务已 COMPLETED 仍为预记」的槽位，与 `SlotSettlement` 同一决策——HasMat 符合阶段预期才 Confirm；明确不符 Rollback+告警；**未知不落账也不回滚预记**（跳过等下轮，工位保持绑定）。未绑加工位（换架等无 PLC 可核）COMPLETED 按方向直接落账。源任务 COMPLETED 的直送登记超宽限（默认 30min）只告警、**不清登记不退中转**；非 COMPLETED 仍按 10min TTL 清除并守恒回收。

**FRAME_BIND**：现场用 `0` 上料 / `1` 下料 / `3` NG，**一架两用**（上游下料 = 下游上料）。`2` 中转仅兼容旧配置，本线不绑。种子：101=内长宽上料；301=内长宽下料=A基准上料；302=A基准下料=平面度上料；303=平面度下料；401=三机 NG。
...
**【v2.3 新增 / v2.4 已实装】同工序多加工位选择策略（三级排序，确定性）**：① 候选 = 状态 WAIT_LOAD 且无排队任务的加工位；② 多候选选**空闲时长最长**者（防饿死、均衡使用）；③ 仍相同按加工位编号升序。**仅上料分配采用此策略**（下料不再选下游空工位），且在单一调度消费者内串行完成（见 §6.4）。
...
- **【2026-09 修订】下发结果未知不按失败处理**（请求已发出但超时/断连/5xx，RCS 可能已建任务）：换架第一发或第二发结果未知时保留事务、进度标"等待 RCS 确认"，任务行置 DISPATCHED 交跟踪器 queryTask 确认，完成事件照常推进；盘点 identifyQR 结果未知时登记为进行中并返回 taskId，扫码结果到达照常回写。跟踪器判定 RCS 侧**查无**时通知换架编排与盘点收口：换架告警并结束事务（第一发：绑定不解除、原状保持；第二发：按站点空置人工处理），盘点发 FAILED 结果。
...
2. **①b 【v2.4】终态收口（防预记残留）**：对未完结集合批量 `queryTask`。已终态的立刻收口槽位账与工位态，并从 active 集合剔除：
   - **COMPLETED**：先 fresh 读 PLC `HasMat`，与运行期同一 `SlotSettlement` 决策——符合阶段预期（上料有料 / 下料无料）才 `ConfirmTake`/`Confirm`；明确不符才 `Rollback*` + Alarm；**HasMat 未知：不落账、不回滚、不回 WaitLoad**，预记与工位绑定保留，等 PLC 可读后再收口（不 latch Alarm，避免启动瞬间误粘滞后再对同槽派工）。未绑加工位 COMPLETED 按方向直接落账。工位：符合 → Loaded / WaitLoad。
...
>
> **【2026-09 修订 · P1-4 内存态重建】** 以下运行态只在内存，重启须从未完结任务重建：
> - **工序间交接登记**：① 绑回下料任务时，若终点（cell，或抓取站码 + ReqParam 抓取孔）反查为下游加工位，补回该工位的交接登记；终点为料架/区域或无法唯一定位则不登记。
> - **换架事务 / 盘点任务**：宿主启动时（先于跟踪器与调度器）按 `TXN_ID` 重建换架事务、按识别任务重建进行中盘点，后续完成/扫码事件照常推进。角色未落库，按缓存区编码反推（仅影响展示与水位去重）。
> - **①b 补发事件**：无工位任务（换架/盘点/回库）在对账中收口为终态时补发一次任务状态事件，与跟踪器轮询发现终态同一路径，否则已接续的换架等不到第一发完成。
> - 未覆盖：停机前第一发已落 COMPLETED 但第二发尚未落库的换架（无未完结行可接续），需人工处理。
...
- 上游多件同时 OK、下一工序中转架空槽不足 → 两件都预占同一槽 → 第二件入库失败转 ALARM。
...
**第二层（数据兜底）· 槽位三态 + 原子条件更新**：`FRAME_SLOT.SLOT_STATE` = `0` 空 / `1` 有料 / `3` 预记（另 `2` 锁定不可用）。调度器先预生成 taskId，再用"候选选取 + 带条件的原子 UPDATE（`WHERE ID=@id AND SLOT_STATE=期望态`）+ 影响行数校验"落预记，只有预记成功才使用同一 taskId 下发 RCS。影响行数=0 即该槽已被并发写入者（另一次派工 / 人工校正 / 盘点回写）抢占 → 重选下一候选，直到成功或无可选。即使消费者之外存在第二写入者，数据库也保证不双占。RCS 下发失败/异常立即补偿回滚；任务 COMPLETED 且 PLC 符合才落账（取料置空 `0` / 入库置占用 `1`）；HasMat 未知不得落账也不得回滚；运行期失败/取消 → 回滚预记（取料回占用、入库回空）。报警撤账后再 Redo / AutoRedo 须先再预记再下发（见 §6.1）。
...
**确定性选位（上料）**：同工序多加工位竞争时按三级排序确定归属——① 候选 = WAIT_LOAD 且无当前任务、无在途交接、未 `UploadRequested` 的加工位；② 空闲时长最长者优先（进入 WAIT_LOAD 的时刻 `WaitLoadSince` 最早，防饿死、均衡）；③ 仍相同按加工位编号升序。选槽与预记同在单消费者内串行完成。上料分配候选仍排除 Pending/Dispatched 任一登记（遗留直送与自取互斥，见 §6.2 v2.5）。
...
**受管派工路由（P0-5）**：自动派工、手动重放、AutoRedo、换架、盘点、回库等**新外部执行**须经路由解析 + 活动校验；Equipment / Craft / WorkLine / LOCATION_MAP / FrameBind / Frame 仅 `STATE=="0"` 可参与。软删、未知、校验失败 → fail-closed 拒发（无 LINE 假码回退、无 Skip 旁路）。历史任务 Confirm/Rollback/Query/对账收口继续既有路径。AutoRedo 顺序：路由门禁 → **补预记** → 原子 Claim → Send；路由拒发或预记失败不消耗 `REDO_COUNT`；Claim 失败才回滚本轮新预记。手动 Redo：门禁 → 补预记 → Increment → Send。
...
| DONE_OK / DONE_NG | OK 或 NG=ON | 记录 + **写 POS_TEST_START=2 关启动** + 下发 RCS 下料（同上料链路：DISPATCHING→TRANSPORTING） | UNLOADED |
| UNLOADED 已下料 | 【v2】RCS 下料完成 且 HasMat **确认 OFF**（未知不通过） | 再写 POS_TEST_START=2（幂等） | WAIT_LOAD |
| ALARM 报警 | 机台安全=OFF / 开门 / 通信断 / 【v2 新增】RCS 任务 failed/canceled/killed、回调超时、warnCallback 严重告警、**RCS 报完成但 PLC 复核不过/未知连续超阈** | 告警，暂停该位调度 | WAIT_LOAD（恢复后；~~【v2.7】人工恢复时按方向回滚本工位预记并清交接~~ **【2026-09 修订】进告警时绑定任务未终态不回滚预记、不清交接；人工恢复须任务已终态：COMPLETED 按 SlotSettlement（PLC 复核）落账或回滚，HasMat 未知拒绝恢复；FAILED/CANCELED 按方向回滚并清交接**）|
...
| 【v2】RCS 任务 canceled/killed | queryTask/回调 | 生成人工处理工单（小车/容器需人工处理），确认前锁定相关点位派工；现场处理后在监控看板确认/恢复 |
...
>
> 【v2.1 开口项回复纪要（2026-07）】① 孔位由上位机指定、物料码经 identifyQR 获取——抓取环节实时账成立；② container 字段 RCS 未处理（C3/C7 未用），托盘级走盘点制；③ 粒度：抓取 station / 搬运 cell，LOCATION_MAP 双层录入；④ 搬运完成经 pushTaskStatus（3.6）推送——回调为主、轮询兜底；⑤ taskType 预留字段。**【2026-09】transitTask.taskType 按契约 `out`/`in`/`move`；工位上下料默认 grabTask（ADR-0004）。**
>
...
>
> 【v2.2 业务方确认（2026-07）】⑩ 工件为 iPad 外壳，串联三工序；**【2026-09 现场】顺序为内长宽→A基准→平面度；无工位直送，料架一架两用（上游下料=下游上料）**（路由决策见 §6.2）；⑪ NG 品入独立 NG 架、不向后流转，人工处理后界面登记闭环；⑫ 满架/空架自动呼叫 AGV 拉走，空托盘也呼叫 AGV 回收，上下料区调试初期人工；⑬ 换架为上位机编排的任务对（先拉后送，第二发挂首发完成事件）；⑭【v2.3】空托盘回收为人工按钮触发（PALLET_RETURN 任务）；多加工位选位策略、换架回滚、启动对账规则已定（§6.2、§6.3）。**待补确认**：缓存区/备料区/托盘回收区点位编码（随 cell 级清单录入）；回调重推机制与告警解除通知、回调监听 IP/端口对接（联调前一周谈）。
...
| 方向 | 角色 | 接口 | 用途 |
| --- | --- | --- | --- |
| 上位机 → RCS | HTTP 客户端 | `POST /api/ExternalInterfaces/transitTask` | 搬运任务（cell 级点位；taskType=`out` 出库/`in` 入库/`move` 搬运；container 不填，托盘由 RCS 自选）。换架/空托盘/命名区回退用 `move`；`LoadUnloadVerb=Transit` 时工位上料 `out`、下料 `in` |
...
| 上位机 → RCS | HTTP 客户端 | `POST /api/ExternalInterfaces/queryTask` | 条件查询（AND/OR + key/value/operator；批量兜底用 `key=Id` + `IN`，value 为**本地 taskId** 逗号列表，如 `L1-GB-20260911142538-9209`；分页；无频率限制） |
...
- **queryTask 现场外包**：外层可能 `Success=false` 且 `Message=成功`，分页 `items` 与内层 `Success` 在 `Data` 中；业务 ACK 与 items 以 `Data` 为准。无 `Data` 内成功证据时，外层 `Success=false` 仍失败。
- `priority`：1~10 大者优先；本系统策略：换架/回收 ≥ 紧急下料 > 常规上料。
- **taskId 双号**：本地先生成 `{线体}-{类型缩写}-{yyyyMMddHHmmss}-{4位序列}`（如 `L1-GB-20260911142538-9209`）并**先落库后发送**，`RCS_TASK_ID` 始终保持该本地号（预记 REMARK / 调度绑定 / redo 同号）。ACK `Data` 带 RCS `task_id`（与 `booking.id` 同值，现场形如 `CNC_WMS_TASK_2_…`）时写入 `RCS_REMOTE_ID`，**不覆盖**本地号。`cancelTask` 传回包号；无回包号（模拟器）仍传本地号。`queryTask` 条件字段为 `Id`，值为本地号。回调按报文 `taskId` 对本地号或回包号落库。
...
| 超时 | 下发 HTTP 超时默认 10s；`TaskExecutionTimeoutMs`（默认 15min）与 `ProcessTimeoutMs`（默认 10min）超时 → Alarm；跟踪器仍按 poll 做 queryTask 兜底 |
| 重试 | 网络级：失败退避重试 ≤3；业务级：commandType=redo，REDO_COUNT 计数；**【P0-5 / ADR-0002】** AutoRedo 须先过路由门禁、再补预记、再原子 Claim；路由拒发或预记失败不耗次数；本轮新锁且下发失败才回滚预记 |
| 轮询 | 兜底通道：2~5s 批量 queryTask（`Id` IN 合并未完结本地号）；与回调冲突以 queryTask 为准 |
