...
| `MAS_AUTO_LOCATION_MAP`（新增） | 逻辑位置（机台/加工位/料架位）↔ RCS 点位编码（`RCS_CODE` + `RCS_TYPE`：shelf/cell/station）。**【现场搬运】cell = 货架码+层号（不补零）+层内位2位（10层×左右2：上料 101101/101102…1011001/1011002；中转 301/302、下料 303、NG 401 同规则；内长宽 201101/201102）；抓取仍用 station/shelf（101/201）。禁止 `LOAD_AREA=101` 与上料架 shelf 同码** |
