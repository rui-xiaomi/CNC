...
**FRAME_BIND**：现场用 `0` 上料 / `1` 下料 / `3` NG，**一架两用**（上游下料 = 下游上料）。`2` 中转仅兼容旧配置，本线不绑。种子：101=内长宽上料；301=内长宽下料=平面度上料；302=平面度下料=A基准上料；303=A基准下料；401=三机 NG。
...
>
> 【v2.2 业务方确认（2026-07）】⑩ 工件为 iPad 外壳，串联三工序（内长宽→平面度→A基准）；**【2026-09 现场】无工位直送，料架一架两用（上游下料=下游上料）**（路由决策见 §6.2）；⑪ NG 品入独立 NG 架、不向后流转，人工处理后界面登记闭环；⑫ 满架/空架自动呼叫 AGV 拉走，空托盘也呼叫 AGV 回收，上下料区调试初期人工；⑬ 换架为上位机编排的任务对（先拉后送，第二发挂首发完成事件）；⑭【v2.3】空托盘回收为人工按钮触发（PALLET_RETURN 任务）；多加工位选位策略、换架回滚、启动对账规则已定（§6.2、§6.3）。**待补确认**：缓存区/备料区/托盘回收区点位编码（随 cell 级清单录入）；回调重推机制与告警解除通知、回调监听 IP/端口对接（联调前一周谈）。
