...
- **同一料架并发保护**：多加工位同时从一个料架取料时，第 2 步选孔位需加锁（数据库行锁或内存互斥），防止两任务选中同一电极。
...
**【v2.3 新增】同工序多加工位选择策略（三级排序，确定性）**：① 候选 = 状态 WAIT_LOAD 且无排队任务的加工位；② 多候选选**空闲时长最长**者（防饿死、均衡使用）；③ 仍相同按加工位编号升序。
...
### 6.3 【v2.3 新增】断电/重启恢复（启动对账）

原则：**宁可保守停下，不做聪明猜测**。启动流程四步，对账完成前禁止自动派工（只收不发）：

1. **任务对账**：查询所有未完结任务（CREATED / DISPATCHED / EXECUTING），分类收敛：
   - `CREATED`（已落库未下发）→ 直接重新入队（taskId 未发出过，天然幂等）；
   - `DISPATCHED / EXECUTING` → queryTask 批量查 RCS 真实状态：completed → 补做 PLC 复核与后续步骤；queued/underway → 恢复跟踪；failed/canceled → 走 §8 既有异常流程；**RCS 查无此任务 → 告警人工**。
2. **状态机重建**：PLC 重连后读全量信号，以"PLC 信号 + 任务对账结果"合成每个加工位初始状态；**账实对不上**（如 PLC 有料但无对应任务记录、或反之）→ 该加工位置 ALARM 挂起，人工确认后恢复。
3. **账目回滚**：槽位处于"预记"状态但对应任务已失败/取消的，回滚预记；换架事务停在中间态的，按 §6.2 回滚规则接续或转工单。
4. **开闸**：以上全部完成后置"对账完成"标志，调度器方可开始自动派工；恢复过程全程记入日志。
...
| 状态 | 进入条件 | 触发动作 | 下一状态 |
| --- | --- | --- | --- |
| WAIT_LOAD 等待上料 | 允许上料=ON 且 有料=OFF | 【v2】生成任务落库、下发 RCS 上料任务 | DISPATCHING |
