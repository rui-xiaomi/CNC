...
补充规则：
- **槽位账时序**：第 2 步选定孔位时"预记"（占用意向，须先于 RCS，见 ADR-0002），第 6 步回调确认且 PLC 符合阶段预期后"落账"。取消/失败回滚预记。Redo / Redispatch / AutoRedo：已有预记幂等跳过，已回滚须先再预记再下发；预记失败拒发；本轮新锁且下发失败才回滚。上料有物料码只锁该件，找不到不得改抢其它槽。命名区 / 换架 / grab / identify 无槽位账则跳过。手工无 taskId 首次下发不在此列。
