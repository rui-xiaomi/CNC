...
CREATE TABLE MAS_AUTO_FRAME_BIND (
  ...
  FRAME_ROLE  CHAR(1) NOT NULL COMMENT '角色 0=上料LOAD 1=下料UNLOAD 2=中转TRANSIT 3=NG架NG_FRAME',
  STATE       CHAR(1) NOT NULL DEFAULT '0',
  AUTHOR      VARCHAR(15) NULL,
  UPDATETIME  DATETIME NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (ID1),
...
CREATE TABLE MAS_AUTO_AGV_TASK (
  ...
  TASK_STATUS     CHAR(1)     NOT NULL DEFAULT '0' COMMENT '[legacy]0=等待 1=执行中 2=完成 3=失败（改用 TASK_STATE）',
  TASK_STATE      VARCHAR(20) NOT NULL DEFAULT 'CREATED' COMMENT '本系统态 CREATED/DISPATCHED/EXECUTING/COMPLETED/FAILED/CANCELED',
  RCS_STATUS      VARCHAR(20) NULL COMMENT 'RCS 原始 11 态 queued/underway/completed/failed/canceled...',
  PRIORITY        INT         NOT NULL DEFAULT 5 COMMENT '优先级 1-10 大者优先',
  FROM_FRAME_CODE VARCHAR(50) NOT NULL COMMENT '取料位标识码/起点 RCS 编码',
  TO_FRAME_CODE   VARCHAR(50) NOT NULL COMMENT '放料位标识码/终点 RCS 编码',
  ...
  DISPATCH_TIME   DATETIME    NULL COMMENT '下发成功时间',
...
CREATE TABLE MAS_AUTO_ALARM_EVENT (
  ...
  EQUIMENT_ID BIGINT      NULL COMMENT '关联机台',
  POSITION_ID BIGINT      NULL COMMENT '关联加工位',
  ALARM_TYPE  VARCHAR(30) NOT NULL COMMENT '告警类型 PLC_OFFLINE/READ_TIMEOUT/UNSAFE/POS_STUCK/FRAME_FULL/RCS_WARN/RCS_TASK_FAIL/PLC_VERIFY_FAIL...',
...
CREATE TABLE MAS_AUTO_LOCATION_MAP (
  ID1         BIGINT      NOT NULL AUTO_INCREMENT COMMENT '主键',
...
-- 18. RCS 双向报文流水
CREATE TABLE MAS_AUTO_RCS_MSG_LOG (
  ID1            BIGINT      NOT NULL AUTO_INCREMENT COMMENT '主键',
