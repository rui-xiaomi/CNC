...
| 料架拆三张表 | `MAS_AUTO_FRAME`（主）+ `MAS_AUTO_FRAME_BIND`（机台绑定，支持一架两用）+ `MAS_AUTO_FRAME_SLOT`（槽位/物料追踪），取代原 `MAS_AUTO_EQUIMENT_MATERIEL_FRAME` 单挂法 |
...
erDiagram
    ...
    FRAME ||--o{ FRAME_SLOT : "槽位/物料"
```

要点：
- `FRAME_BIND` 是料架与机台的**多对多 + 角色**关系，一行 `(料架,机台,角色)`。一架两用 = 同一 `FRAME_ID` 出现在两行不同角色/机台中（现场：301=内长宽下料+A基准上料，302=A基准下料+平面度上料）。`FRAME_ROLE`：`0=上料 1=下料 2=中转（本线不绑） 3=NG`。
...
**MAS_AUTO_FRAME_SLOT（分层 + 物料绑定）**：槽位带两级物理坐标 `LAYER_NO`（层）+ `POS_IN_LAYER`（层内位），另有料架内全局槽号 `SLOT_NO`；唯一键同时约束 `(FRAME_ID,SLOT_NO)` 与 `(FRAME_ID,LAYER_NO,POS_IN_LAYER)`。`ELECTRODE_ID`+这些坐标构成"物料A在中转架1层4位"的绑定。`SLOT_STATE`：`0` 空 / `1` 占用 / `2` 锁定 / `3` 预记。预记时 `REMARK`=taskId、`BIND_SOURCE`=`RSV_TAKE`/`RSV_PUT`；落账后 `BIND_SOURCE`=`CONFIRMED`（REMARK 可保留作幂等）。`idx_slot_electrode` 支持按物料ID快速反查。`idx_slot_frame_state (FRAME_ID, SLOT_STATE)` 支持“按料架+状态”选候选槽，`idx_slot_remark (REMARK)` 支持按 taskId 回查预记槽。Redo 按 taskId 查仍有效的预记；上料补预记可按物料码只锁该占用槽。
...
## 5b. 第四阶段 RCS 对接相关表（已并入 `sql/cnc_schema.sql`）

**MAS_AUTO_AGV_TASK（扩展为 RCS 任务记录）**：承载一条 RCS 任务的完整生命周期。
- `RCS_TASK_ID`：本地下发号 `{线体}-{类型}-{yyyyMMddHHmmss}-{4位序列}`（如 `L1-GB-20260911142538-9209`），下发 / 查询 / redo / 预记 REMARK 用此号，ACK 后**不覆盖**。
- `RCS_REMOTE_ID`：ACK `Data.task_id`（现场 `CNC_WMS_TASK_2_…`），仅 `cancelTask` 使用。无回包号时为空，取消回退本地号。料架路径重发时槽位须仍为本地 taskId 的预记，或先再预记再下发（见开发文档 §6.1、ADR-0002）。
- **唯一约束**：`RCS_TASK_ID` → `uk_agv_rcs_task`、`RCS_REMOTE_ID` → `uk_agv_rcs_remote`，由数据库层兜底 taskId 唯一（回调去重、跟踪器查询、对账都以其唯一为前提）。唯一键允许多个 NULL。结构以 `sql/cnc_schema.sql` 为准。
...
**MAS_AUTO_LOCATION_MAP（逻辑位置↔RCS点位编码）**：把机台/加工位/料架/命名区域映射到 RCS 点位编码。`RCS_TYPE ∈ {shelf,cell,station}`——**抓取用 station/shelf**（上料架 101 / 内长宽 201 / 中转 301·302 / NG 401），**搬运用 cell**（料架：货架码+层编码从10起+层内位1位：10 层×左右 2，上料 L1=101101/101102、L2=101111/101112、L10=101191/101192；中转 301/302、下料 303、NG 401 同规则；内长宽工位1=201101、工位2=201102）。上料架物料码与所在槽 cell 相同。`LOAD_AREA`/`UNLOAD_AREA` 不得与料架 shelf 同码（解析器会歧义拒发）。缓存区/回收区用 `LOC_TYPE='AREA'` + `LOC_NAME`。

**MAS_AUTO_RCS_MSG_LOG（双向报文流水）**：`DIRECTION` OUT=出站请求 / IN=入站回调；记 `INTERFACE_NAME`、`URL`、`TASK_ID`、请求/应答报文原文、耗时、结果，供联调追溯与幂等去重。

**MAS_AUTO_WORKLINE_AGV（RCS 连接配置，见 `sql/cnc_schema.sql`）**：正式存 RCS 出站与回调。列含 `RCS_BASE_URL` / `RCS_CLIENT_CODE` / `RCS_CALLBACK_HOST` / `RCS_CALLBACK_PORT` / `RCS_TIMEOUT_MS` / `RCS_MAX_RETRIES` / `RCS_POLL_INTERVAL_MS`。旧 `AGV_COMPUTER_*` 保留兼容；口令列本系统不读写加密。页面「连接&下发」可编辑保存；出站字段热更新，回调 Host/Port 变更需重启。

**MAS_AUTO_FRAME_SLOT 扩展**：`BIND_SOURCE`（预记 `RSV_TAKE`/`RSV_PUT`；落账 `CONFIRMED`；物料码绑定 `MANUAL/SCAN_GUN/RCS_QR`）、`LAST_VERIFY_TIME`（最近 identifyQR 盘点校正时间，物料反查结果附带数据新鲜度）。预记绑定见上文槽位三态。

## 6. 初始化数据（示例场景）

`sql/cnc_schema.sql` 末尾内置示例：**1 线体 → 3 道工序 → 3 机台（内长宽→A基准→平面度）**；**3 台逻辑 PLC**（一机一 `PLC_ID`），种子默认 FINS `192.168.250.1:9600`（三行 IP 相同，表示共物理 PLC；演示 `UseSimulator=true` 时仍各起本机模拟端口）。点位按《测试机信号表》分属各逻辑 PLC。机台站码 201/202/203 按工序顺序。
