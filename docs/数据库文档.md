...
**MAS_AUTO_FRAME_SLOT（分层 + 物料绑定）**：槽位带两级物理坐标 `LAYER_NO`（层）+ `POS_IN_LAYER`（层内位），另有料架内全局槽号 `SLOT_NO`；唯一键同时约束 `(FRAME_ID,SLOT_NO)` 与 `(FRAME_ID,LAYER_NO,POS_IN_LAYER)`。`ELECTRODE_ID`+这些坐标构成"物料A在中转架1层4位"的绑定，`SLOT_STATE` 标空/占用。`idx_slot_electrode` 支持按物料ID快速反查。
...
**MAS_AUTO_FRAME_SLOT 扩展**：`BIND_SOURCE`（`MANUAL/SCAN_GUN/RCS_QR` 物料码绑定来源）、`LAST_VERIFY_TIME`（最近 identifyQR 盘点校正时间，物料反查结果附带数据新鲜度）。
