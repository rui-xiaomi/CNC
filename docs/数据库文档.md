...
**MAS_AUTO_FRAME_SLOT（分层 + 物料绑定）**：槽位带两级物理坐标 `LAYER_NO`（层）+ `POS_IN_LAYER`（层内位），另有料架内全局槽号 `SLOT_NO`；唯一键同时约束 `(FRAME_ID,SLOT_NO)` 与 `(FRAME_ID,LAYER_NO,POS_IN_LAYER)`。`ELECTRODE_ID`+这些坐标构成"物料A在中转架1层4位"的绑定。`SLOT_STATE`：`0` 空 / `1` 占用 / `2` 锁定 / `3` 预记。预记时 `REMARK`=taskId、`BIND_SOURCE`=`RSV_TAKE`/`RSV_PUT`；落账后 `BIND_SOURCE`=`CONFIRMED`（REMARK 可保留作幂等）。`idx_slot_electrode` 支持按物料ID快速反查。Redo 按 taskId 查仍有效的预记；上料补预记可按物料码只锁该占用槽。
...
- `RCS_REMOTE_ID`：ACK `Data.task_id`（现场 `CNC_WMS_TASK_2_…`），仅 `cancelTask` 使用。无回包号时为空，取消回退本地号。已有库执行 `docs/sql/alter_agv_task_remote_id.sql`。料架路径重发时槽位须仍为本地 taskId 的预记，或先再预记再下发（见开发文档 §6.1、ADR-0002）。
