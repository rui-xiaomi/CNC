...
**WPF 条件分支**：若项目满足 WPF 识别条件，先读取 [WPF 平台契约](../wpf-desktop-workflow/references/WPF-PROFILE.md)。将“页面清单”替换为“Surface 清单”，使用 `surface-id`、Owner、模态性、生命周期、焦点、键盘、DPI/主题、状态适用性和测试接缝。兼容下游旧字段时令 `page-id = surface-id`。状态策略使用桌面候选十态，但只要求每个 Surface 明确适用项与不适用理由。不得要求 DOM/CSS/ARIA、浏览器路由、Playwright 或 PNG 作为 WPF 默认验收。
