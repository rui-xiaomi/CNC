...
若项目满足 WPF 识别条件，先读取 [WPF 平台契约](../wpf-desktop-workflow/references/WPF-PROFILE.md) 和 [WPF UI Issue 模板](../wpf-desktop-workflow/references/WPF-ISSUE-TEMPLATE.md)。推到 `ready-for-agent` 前检查：`surface-id`、Owner/模态性、app-shell 依赖、适用 States、AutomationProperties、ViewModel/STA/UI Automation/人工 QA 四层计划，以及交互式 Windows Session 要求。不要套用 Web CSS/ARIA/路由/PNG 门禁。
