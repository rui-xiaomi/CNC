...
若项目满足 WPF 识别条件，先读取 [WPF 平台契约](../wpf-desktop-workflow/references/WPF-PROFILE.md)。WPF UI Issue 不使用 `ui-issues.md` 中的 DOM/CSS/ARIA、浏览器路由、Playwright 或像素稿默认值；测试顺序固定为领域/ViewModel → STA WPF 集成 → 关键路径 UI Automation → 人工桌面 QA。每层均运行项目真实命令并分别报告结果。UI Automation 需要交互式 Windows Session 时必须显式说明；缺少该会话不得声称通过。
