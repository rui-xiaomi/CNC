...
若项目满足 WPF 识别条件，先读取 [WPF 平台契约](../wpf-desktop-workflow/references/WPF-PROFILE.md) 和 [WPF UI Issue 模板](../wpf-desktop-workflow/references/WPF-ISSUE-TEMPLATE.md)。WPF UI Issue 使用 `surface-id`、Owner/模态性、States 矩阵和四层验证，不使用下文 Web 专属的 CSS/ARIA/路由/Playwright/PNG 默认项。兼容旧流程时令 `page-id = surface-id`。`app-shell` 仍为第一优先依赖；对话框、托盘、向导、浮动工具窗也各自作为 Surface 切片。
